...
async function runTest(test_url, input_data = {}) {
  ...
  // Fill form with input data (business email from ceca)
  await sleepFunc();
  ...
  if (currentUrl.includes('my-compliance')) {
    ...
    try {
      ...
      console.log('PASS: Company email lead found as expected in Salesforce.');
      await browser.close();
      process.exit(0);
    } catch (error) {
      // If any step fails (e.g., timeout), lead not found, which is FAIL
      await takeScreenshotFunc();
      console.log('FAIL: Company email lead not found in Salesforce.');
      await browser.close();
      process.exit(1);
    }
  } else {
