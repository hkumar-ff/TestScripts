...
async function runTest(test_url) {
  ...
  await sleepFunc();
  await chooseOptions(page, browser, "Who are you seeking compliance for?", "Step 2: Compliance Subject", ["I need to evaluate my vendor for compliance"], sleepFunc, takeScreenshotFunc);
  ...
  // Step 4: Environment
  await sleepFunc();
  await chooseOptions(page, browser, "Please select the environment that applies to your vendor", "Step 4: Environment", ["Azure Gov Cloud"], sleepFunc, takeScreenshotFunc);
  await clickNext(page, browser, "Step 4: Environment", sleepFunc, takeScreenshotFunc);

  // Step 5: IT Footprint
  await sleepFunc();
  await chooseOptions(page, browser, "Please select the closest that apply to your vendor", "Step 5: IT Footprint", ["Less than 50", "2 or less", "6 to 25"], sleepFunc, takeScreenshotFunc);
  await clickNext(page, browser, "Step 5: IT Footprint", sleepFunc, takeScreenshotFunc);

  // Step 6: Familiarity With Compliance
  await sleepFunc();
  await chooseOptions(page, browser, "Please select the closest that apply to your vendor", "Step 6: Familiarity With Compliance", ["My vendor has been certified in past to an IT standard like SOC 2 or ISO 27001"], sleepFunc, takeScreenshotFunc);
  ...
  }, "because you selected that your vendor has been formally audited in the past. Please edit the pre-filled questions as needed if not accurate.");
  if (!isCheckTextPresentStep7) {
    console.log(`Fail: Step 7: IT Controls: because you selected that your vendor has been formally audited in the past. Please edit the pre-filled questions as needed if not accurate. not found on the page`);
    await browser.close();
    process.exit(1);
  }
  console.log(`Pass: Step 7: IT Controls: because you selected that your vendor has been formally audited in the past. Please edit the pre-filled questions as needed if not accurate.`);
  ...
  const isProposalPresent = await page.evaluate((text) => {
    return Array.from(document.querySelectorAll('*')).some(el => el.textContent.includes(text));
  }, "CMMC C3PAO Certification Proposal");
  if (!isProposalPresent) {
    console.log('Fail: CMMC C3PAO Certification Proposal not found on the page');
    await browser.close();
    process.exit(1);
  }
  console.log('Pass: CMMC C3PAO Certification Proposal');

  const isForVendorPresent = await page.evaluate((text) => {
    return Array.from(document.querySelectorAll('*')).some(el => el.textContent.includes(text));
  }, "For Your Vendor");
  if (!isForVendorPresent) {
    console.log('Fail: For Your Vendor not found on the page');
    await browser.close();
    process.exit(1);
  }
  console.log('Pass: For Your Vendor');
