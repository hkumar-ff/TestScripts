...
async function runTest(targetUrl) {
  ...
  try {
    ...
    // Find and click "Customize Your Plate" link
    await sleepFunc();
    await safeClickByText(page, ['customize your plate'], 'Customize Your Plate');
    console.log('Clicked "Customize Your Plate" link');

    // Wait for page to load
    await waitForPageLoad(page);
    console.log('Customize Your Plate page loaded');
    ...
    await page.screenshot({ path: `test_customize_error_${Date.now()}.png`, fullPage: true }).catch(() => {});
    await browser.close();
    process.exit(1);
  }
}
